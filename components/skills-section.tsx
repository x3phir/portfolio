...
function AzureIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      <path d="M8.7 3h6.1L8.5 21H2.8L8.7 3Z" />
      <path d="M15.5 3 21.2 21h-6.1l-1.1-3.9H8.9l7.4-8.5-.8-5.6Z" />
    </svg>
  )
}
...
export function SkillsSection() {
  return (
    <section id="skills" className="bg-background px-6 py-24 md:py-32">
      <div className="mx-auto max-w-5xl">
        <AnimatedContent distance={18} duration={850}>
          <p className="mb-16 text-sm font-medium md:mb-24">skills.</p>
        </AnimatedContent>
          ...
          <AnimatedContent delay={80} distance={26} duration={950}>
            ...
            <div className="flex flex-wrap justify-center gap-8 md:gap-12 px-30">
              {languages.map((item) => (
                <div
                  key={item.name}
                  className="flex h-10 w-10 items-center justify-center transition-transform duration-300 ease-out hover:-translate-y-1 hover:scale-110 md:h-12 md:w-12"
                  title={item.name}
                >
